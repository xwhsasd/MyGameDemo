...
public class Inventory : MonoBehaviour, ISaveManager
{
    ...
    [SerializeField]private PickItem_Tooltip[] itemTooltips;
    private bool openItemTooltip = true;
    ...
    private void Start()
    {
        ...
        AddStartingItems();
        openItemTooltip = true;
    }

    private void AddStartingItems()
    {
        openItemTooltip = false;
    ...
    public void AddItem(ItemData _item)
    {
        if (_item.itemType == ItemType.Equipment && CanAddItem())
            ...
            AddToStash(_item);
        OpenItemTooltips(_item);
        UpdateSlotUI();
    }


    private void OpenItemTooltips(ItemData _item)
    {
        if (openItemTooltip == false)
            return;
        for (int i = itemTooltips.Length - 1; i >= 0; i--)
        {
            if (itemTooltips[i].canBeFilled)
            {
                itemTooltips[i].gameObject.SetActive(true);
                itemTooltips[i].GetInfo(_item);
                itemTooltips[i].canBeFilled = false;
                return;
            }
        }
    }
    ...
    public void RemoveItem(ItemData _item)
    {
        ...
        if (stashDictianory.TryGetValue(_item, out InventoryItem stashVale))
        {
            if (stashVale.stackSize <= 1)
            {
                stash.Remove(stashVale);
                stashDictianory.Remove(_item);
            }
            else
                stashVale.RemoveStack();
        }
